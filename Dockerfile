...
RUN \
    ...
    ./configure --prefix=/usr \
      ...
      --without-javac --without-wx --without-debugger --without-observer --without-jinterface \
      --without-cosEvent --without-cosEventDomain --without-cosFileTransfer --without-cosNotification \
      --without-cosProperty --without-cosTime --without-cosTransactions --without-et --without-gs \
      --without-ic --without-megaco --without-orber --without-percept --without-typer \
      --enable-threads --enable-shared-zlib --enable-ssl=dynamic-ssl-lib --enable-hipe && \
      # Build
...
COPY --from=erlang-builder /usr/lib/erlang /usr/lib/erlang
...
RUN apk --no-cache --virtual .elixir-install add wget && \
    ...
    unzip Precompiled.zip -d /usr/lib/elixir/ && \
    rm Precompiled.zip && \
    for F in elixir elixirc iex mix; do \
      ln -s /usr/lib/elixir/bin/${F} /usr/bin/${F}; \
    done && \
    mix local.hex --force && \
